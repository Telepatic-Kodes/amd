...
export function ReportSettings() {
  ...
  return (
    <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-6 space-y-6">
      {/* Title */}
      <h3 className="text-lg font-semibold text-zinc-100">{LABELS.reportSettings}</h3>

      {/* Email Delivery Toggle */}
      <div className="flex items-center justify-between">
        <div>
          <p className="text-sm font-medium text-zinc-300">{LABELS.emailDelivery}</p>
          <p className="text-xs text-zinc-500">{LABELS.emailDeliveryDesc}</p>
        </div>
        <button
          onClick={() => handleToggleEmail(!settings.emailEnabled)}
          className={`w-11 h-6 rounded-full transition-colors ${
            settings.emailEnabled ? "bg-indigo-500" : "bg-zinc-700"
          } relative`}
        >
          <div
      ...
      {/* Frequency Selector */}
      <div>
        <p className="text-sm font-medium text-zinc-300 mb-2">{LABELS.reportFrequency}</p>
      ...
      <div className="flex items-center justify-between">
        <div>
          <p className="text-sm font-medium text-zinc-300">{LABELS.includeAiInsights}</p>
          <p className="text-xs text-zinc-500">{LABELS.includeAiInsightsDesc}</p>
        </div>
        <button
          onClick={() => handleToggleNarrative(!settings.includeNarrative)}
          className={`w-11 h-6 rounded-full transition-colors ${
            settings.includeNarrative ? "bg-indigo-500" : "bg-zinc-700"
          } relative`}
        >
          <div
            className={`absolute top-0.5 w-5 h-5 bg-white rounded-full transition-transform ${
      ...
      <div>
        ...
        <input
          ...
          className="w-full bg-zinc-800 border border-zinc-700 text-zinc-100 placeholder-zinc-500 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-1 focus:ring-indigo-500"
        />
        <p className="text-xs text-zinc-500 mt-1">{LABELS.recipientEmailDesc}</p>
      </div>

      {/* Manual Trigger Section */}
      <div className="border-t border-zinc-800 pt-4 mt-4">
        <p className="text-sm font-medium text-zinc-300 mb-3">{LABELS.generateNow}</p>
        <div className="flex gap-2">
          <button
            onClick={() => handleGenerateNow("weekly")}
            disabled={generating === "weekly"}
            className="flex-1 flex items-center justify-center gap-2 bg-indigo-600 hover:bg-indigo-700 disabled:bg-indigo-800 disabled:cursor-not-allowed text-white text-sm font-medium py-2 px-4 rounded-lg transition-colors"
          >
            {generating === "weekly" ? (
              <>

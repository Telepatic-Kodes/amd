...
function getEngagementRateColor(rate: number | null | undefined): string {
  if (!rate) return "text-[var(--text-tertiary)]";
  if (rate > 3) return "text-[var(--success)]";
  ...
  if (isLoading) {
    return (
      <div className="space-y-3">
        {[...Array(5)].map((_, i) => (
          <div key={i} className="h-12 rounded-lg bg-[var(--surface-1)]/50 animate-pulse" />
        ))}
      </div>
    );
  }
        ...
        <tbody>
          {data.map((item, _i) => {
            ...
            return (
              <tr
                key={item.contentId}
                className="border-b border-[var(--border)]/50 last:border-0 hover:bg-[var(--surface-1)]/30 transition-colors"
              >
                <td className="py-3 pl-6">
                  <div className="flex flex-col gap-1 max-w-md">
                    <span className="text-sm text-[var(--text-secondary)] font-medium truncate">
                      {item.title.length > 50
                        ? item.title.substring(0, 50) + "..."
                        : item.title}
                    </span>
                    {!hasEngagement && (
                      <span className="text-xs text-[var(--text-secondary)]">Sin datos de LinkedIn</span>
                    )}
                  </div>
                </td>
                <td className="py-3">
                ...
                <td className="py-3 text-center">
                  <div className="flex items-center justify-center gap-1.5">
                    <Heart className="w-3.5 h-3.5 text-[var(--error)]" />

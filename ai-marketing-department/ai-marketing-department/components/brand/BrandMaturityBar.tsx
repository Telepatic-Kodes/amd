...
const levelLabels: Record<string, { label: string; color: string }> = {
  ...
  enriched: { label: "Enriquecida", color: "text-blue-400" },
};
        ...
        <div className="flex items-center gap-3">
          ...
          <span
            className={cn(
              ...
              level === "enriched" && "bg-blue-500/10 border-blue-500/20 text-blue-400"
            )}
          >
            {levelConfig.label}
          </span>
        ...
        {breakdown.map((section) => (
          <div key={section.name} className="flex-1 group relative">
            <div className="flex flex-col items-center gap-1">
              ...
              />
              <span className="text-[10px] text-gray-400 group-hover:text-gray-500 transition-colors truncate">
                {sectionLabels[section.name] || section.name}
              </span>
            </div>
            {/* Tooltip on hover */}
            <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 px-2 py-1 rounded bg-gray-100 border border-gray-300 text-xs text-gray-700 opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none whitespace-nowrap z-10">
